          ...
          max_low_rscu_ratio(max_low_rscu_ratio)
    {
        ...
        for (const char c : this->init_target_protein)
        {
            ...
            { // if c not in skip_aa
                ...
                decidable_dna_len += this->tables.reduced_codon_lengths_table.at(c);
            }
        }
    }
};
...
public:
    ...
    SIRIUSConfig(
        ...
        bool fix_variables_to_their_hinted_value = true)
    {
...
public:
    ...
    void solve_model()
    {
        std::cout << BLUE << "> " << RESET << "[" << elapsed_since_start() << "] Solve this enigma...\n";
    ...
    {
        int n_sequences = this->instance.n;

        for (int k = 0; k < n_sequences - 1; ++k)
        {   
            const auto &seq_k_codons = this->sequence_codons_list[k];
            const auto &seq_k1_codons = this->sequence_codons_list[k + 1];

            int seq_length = seq_k_codons.size();
            ...
            for (int pos = 0; pos < seq_length; ++pos)
            {
                ...
                const auto &seq_k_pos_codons = seq_k_codons.at(pos);
                const auto &seq_k1_pos_codons = seq_k1_codons.at(pos);

                int num_codons = seq_k_pos_codons.size();

                // Determine if seq[k] and seq[k+1] codons at this position are equal
                prefix_equal[pos] = this->cp_model.NewBoolVar().WithName(absl::StrFormat("prefix_equal_k%d_pos%d", k, pos));

                // Equality at this position across all codon variables
                std::vector<operations_research::sat::BoolVar> codon_matches_at_pos;
                codon_matches_at_pos.reserve(num_codons);

                for (int c = 0; c < num_codons; ++c)
                {
                    operations_research::sat::BoolVar codon_match_c = this->cp_model.NewBoolVar().WithName(
                        absl::StrFormat("codon_match_k%d_pos%d_c%d", k, pos, c));

                    this->cp_model.AddEquality(seq_k_pos_codons[c], seq_k1_pos_codons[c]).OnlyEnforceIf(codon_match_c);
                    this->cp_model.AddNotEqual(seq_k_pos_codons[c], seq_k1_pos_codons[c]).OnlyEnforceIf(codon_match_c.Not());

                    codon_matches_at_pos.push_back(codon_match_c);
                }
                // `prefix_equal[pos]` is true if and only if ALL `codon_matches_at_pos` are true.
                this->cp_model.AddBoolAnd(codon_matches_at_pos).OnlyEnforceIf(prefix_equal[pos]);

                std::vector<operations_research::sat::BoolVar> negated_codon_matches_at_pos;
                negated_codon_matches_at_pos.reserve(num_codons);

                for (const auto& var : codon_matches_at_pos) {
                    negated_codon_matches_at_pos.push_back(var.Not());
                }
                // And `prefix_equal[pos]` is false if and only if AT LEAST ONE `codon_matches_at_pos` is false.
                this->cp_model.AddBoolOr(negated_codon_matches_at_pos).OnlyEnforceIf(prefix_equal[pos].Not());

                operations_research::sat::BoolVar lex_less_or_equal = this->cp_model.NewBoolVar().WithName(
                    absl::StrFormat("lex_le_k%d_pos%d", k, pos));

                this->cp_model.AddImplication(prefix_equal[pos].Not(), lex_less_or_equal);

                // Ensures prefix_equal[pos] = AND(codon_matches_at_pos)
                ...
                selector_vars.reserve(static_cast<size_t>(num_codons) * num_codons);
                ...
                for (int i = i_wrapping_indexer; i < pattern_end_ranges.at(k); ++i)
                {
                    ...
                    {
                        print_warning_newline("i=" + std::to_string(i) + " j=" + std::to_string(j));

                        operations_research::sat::BoolVar selector = this->cp_model.NewBoolVar().WithName(
                            absl::StrFormat("selector_k%d_pos%d_i%d_j%d", k, pos, i, j));
                        selector_vars.push_back(selector);

                        this->cp_model.AddEquality(seq_k_pos_codons[i], 1).OnlyEnforceIf({lex_less_or_equal, selector});
                        this->cp_model.AddEquality(seq_k1_pos_codons[i], 0).OnlyEnforceIf({lex_less_or_equal, selector});
                        this->cp_model.AddEquality(seq_k_pos_codons[j], 0).OnlyEnforceIf({lex_less_or_equal, selector});
                        this->cp_model.AddEquality(seq_k1_pos_codons[j], 1).OnlyEnforceIf({lex_less_or_equal, selector});

                        // Enforce the constraint group only if this selector is active
                        // this->cp_model.AddGreaterThan(seq_k_pos_codons[i], seq_k1_pos_codons[i])
                        //     .OnlyEnforceIf({lex_less_or_equal, selector});

                        // this->cp_model.AddLessThan(seq_k_pos_codons[j], seq_k1_pos_codons[j])
                        //     .OnlyEnforceIf({lex_less_or_equal, selector});
                        ...
                        //     if (bi != i && bi != j)
                        //     {
                        //         this->cp_model.AddEquality(seq_k_pos_codons[bi], 0).OnlyEnforceIf({lex_less_or_equal, selector});
                        //         this->cp_model.AddEquality(seq_k1_pos_codons[bi], 0).OnlyEnforceIf({lex_less_or_equal, selector});
                        //         // this->cp_model.AddLessOrEqual(seq_k_pos_codons[bi], seq_k1_pos_codons[bi])
                        //         //     .OnlyEnforceIf({lex_less_or_equal, selector});
                        //     }
                        // }
                    }
                ...
                operations_research::sat::LinearExpr selector_sum = 
                    operations_research::sat::LinearExpr::Sum(selector_vars);
                ...
                this->cp_model.AddEquality(selector_sum, 0).OnlyEnforceIf(lex_less_or_equal.Not());
            }
        }
    }
    ...
    VectorColumnYVars create_y_chained_vars()
    {
        ...
        // Pre-allocate capacity for the outer vector (all_pairs_y_terms).
        // The number of unique pairs (s, t) where t > s is n * (n - 1) / 2.
        if (this->instance.n > 1)
        { // Avoid division by zero if n <= 1
            all_pairs_y_terms.reserve(static_cast<size_t>(this->instance.n) * (this->instance.n - 1) / 2);
        }

        for (int s = 0; s < this->instance.n; ++s)
        {
            const auto &s_sequence_vars = this->sequence_vars_list.at(s);

            for (int t = s + 1; t < this->instance.n; ++t)
            {
                VectorLinearExprs this_pair_y_terms;

                size_t estimated_inner_size = 0;
                for (int aa_pos_idx = 0; aa_pos_idx < this->decidable_protein_length; ++aa_pos_idx)
                {
                    estimated_inner_size += this->tables.reduced_codon_lengths_table.at(this->decidable_protein.at(aa_pos_idx));
                }
                this_pair_y_terms.reserve(estimated_inner_size);

                const auto &t_sequence_vars = this->sequence_vars_list.at(t);

                for (int aa_pos_i = 0; aa_pos_i < this->decidable_protein_length; ++aa_pos_i)
                {
                    const int current_aa_codon_length = this->tables.reduced_codon_lengths_table.at(this->decidable_protein.at(aa_pos_i));
                    const auto &s_aa_pos_vars = s_sequence_vars.at(aa_pos_i);
                    const auto &t_aa_pos_vars = t_sequence_vars.at(aa_pos_i);

                    for (int codon_i = 0; codon_i < current_aa_codon_length; ++codon_i)
                    {
                        operations_research::sat::LinearExpr y_terms; // Accumulates z terms for this (s,t,aa_pos_i,codon_i) combination

                        for (size_t codon_position_i = 0; codon_position_i < s_aa_pos_vars.size(); ++codon_position_i)
                        {
                            const auto &x_vars_at_pos_i = s_aa_pos_vars.at(codon_position_i);
                            operations_research::sat::BoolVar x = x_vars_at_pos_i.at(codon_i);

                            for (size_t codon_position_j = 0; codon_position_j < t_aa_pos_vars.size(); ++codon_position_j)
                            {
                                const auto &y_vars_at_pos_j = t_aa_pos_vars.at(codon_position_j);
                                operations_research::sat::BoolVar y = y_vars_at_pos_j.at(codon_i);

                                std::string var_name = x.Name() + y.Name();
                                operations_research::sat::BoolVar z = cp_model.NewBoolVar().WithName(var_name);

                                // This is equivalent to:
                                // If names match, z = x * y (product of binary variables).
                                // If names don't match, z = 0.
                                ...
                                // Add z to the current LinearExpr (y_terms).
                                y_terms += z;
                                // Add z to the global list of all created variables.
    ...
    //     int dna_size = this->instance.decidable_dna_len;
    ...
    //     {
    //         int new_weight = (dna_size - (i - 1)) * n_choose_two * weights[i - 1] + 1;
    ...
    std::vector<operations_research::sat::IntVar> create_additive_objective()
    {
        ...
        {
            operations_research::sat::IntVar y_current = operations_research::sat::IntVar(this->all_pairs_z_terms.at(i).front());
            factored_terms.push_back(y_current); // Add the initial term to the result vector.

            for (int j = 1; j < this->all_pairs_z_terms.at(i).size(); ++j)
            {
                operations_research::sat::BoolVar x = this->all_pairs_z_terms.at(i).at(j);
                operations_research::sat::IntVar z = this->cp_model.NewIntVar({0, j + 1}).WithName(x.Name() + "_product_chain_" + std::to_string(j));
                
                this->cp_model.AddMultiplicationEquality(z, x, operations_research::sat::LinearExpr(1) + y_current);

                y_current = z;                       // Update y_current for the next iteration in the chain.
                factored_terms.push_back(y_current); // Add the newly created 'z' (now y_current) to the result.
            }
        }
...
void check_response(const operations_research::sat::CpSolverResponse &response)
{
    ...
    {
        std::cout << BLUE << "> " << RESET << "[" << elapsed_since_start() << "] Solution is not proven optimal, but feasible.\n";
        std::cout << BLUE << "> " << RESET << "[" << elapsed_since_start() << "] Objective value: " << response.objective_value() << std::endl;
    }
