...
void print_info_newline_time(const std::string &message);
...
int gcd(int a, int b);
int lcm(int a, int b);
int lcm_vector(const std::vector<int>& nums);
std::string get_initial_sequence(std::string prot_seq, std::unordered_map<char, std::vector<std::string>> reduced_codon_table);
...
public:
    ...
    std::vector<std::vector<std::string>> shifted_seqs;
    ...
    void build_model(std::string cycle_start_seq, int n_so_far, int chunk_size, bool find_next_cycle_start)
    {
        int chunk_size_for_solver = n_so_far;
        if (find_next_cycle_start)
        {
            ++chunk_size_for_solver;
        }

        print_info_newline_time("Construct a model...");
        ...
        //                                ? add_base_variables_from_rscu(n)
        //                                : add_base_variables_from_prot(n);

        print_info_newline_time("Creating base vars...");
        this->sequence_vars_list = add_base_variables_from_prot(chunk_size_for_solver);

        print_info_newline_time("Creating codon constraints...");
        this->sequence_codons_list = add_codon_constraints();

        print_info_newline_time("Creating Y chained vars...");
        this->all_pairs_y_terms = create_y_chained_vars(chunk_size_for_solver);

        print_info_newline_time("Creating Z chained vars..");
        this->all_pairs_z_terms = create_z_chained_vars();

        print_info_newline_time("Creating shifted seqs...");
        std::vector<std::vector<std::string>> new_shifted_seqs = 
            generate_shifted_sequences(cycle_start_seq, chunk_size);
        this->shifted_seqs.insert(
            this->shifted_seqs.end(),
            new_shifted_seqs.begin(),
            new_shifted_seqs.end()
        );

        print_info_newline_time("Assign var vals from shifted seqs...");
        assign_var_values_from_solution(shifted_seqs, n_so_far);

        print_info_newline_time("Fix shifted seqs var vals...");
        fix_shifted_var_vals(chunk_size_for_solver, find_next_cycle_start);

        print_info_newline_time("Creating additive objective...");
        this->additive_obj_mults = create_additive_objective();
    }

    void set_minimize_objective_value()
    {
        print_info_newline_time("Setting minimize objective...");
    ...
    void solve_model()
    {
        print_info_newline_time("Solve this enigma...");
    ...
    void fix_shifted_var_vals(int n, bool skip_last_seq)
    {
        int base_vars_end_range = this->sequence_vars_list.size();
        int codon_vars_end_range = this->sequence_codons_list.size();

        if (skip_last_seq)
        {
            --base_vars_end_range;
            --codon_vars_end_range;
        }

        int counts = 0;
        // Base vars
        for (int i = 0; i < base_vars_end_range; ++i)
        {
            const auto &a = this->sequence_vars_list.at(i);
            for (const auto &b : a)
            {
                for (const auto &c : b)
                {
                    ...
                    {
                        ++counts;
                        if (this->map_var_name_to_val.find(var.Name()) != this->map_var_name_to_val.end())
                        {
                            this->cp_model.AddEquality(var, this->map_var_name_to_val.at(var.Name()));
                        }
                        else
                        {
                            this->cp_model.AddEquality(var, 0);
                        }
                    }
                }
            }
        ...
        // Codon multiples
        // for (int i = 0; i < codon_vars_end_range; ++i)
        // {
        //     const auto &a = this->sequence_codons_list.at(i);
        //     for (const auto &b : a)
        //     {
        ...
        //     for (int t = s + 1; t < n; ++t)
        //     {
        //         if (skip_last_seq && (t == n - 1))
        //         {
        //             ++index;
        //             continue; // Skip pairs with t == last sequence
        //         }

        //         const auto &z_bools = this->all_pairs_z_terms.at(index);
        //         for (const auto &var : z_bools)
        //         {
        //             if (this->map_var_name_to_val.find(var.Name()) != this->map_var_name_to_val.end())
        //             {
        //                 this->cp_model.AddEquality(var, this->map_var_name_to_val.at(var.Name()));
    ...
    void assign_var_values_from_solution(std::vector<std::vector<std::string>> shifted_seqs_solutions, int n)
    {
        int colcount = 0;
        for (int s = 0; s < n; ++s)
        {
            for (int t = s + 1; t < n; ++t)
            {
                ...
                {
                    std::string x = shifted_seqs_solutions.at(s).at(aa_pos_i);
                    std::string y = shifted_seqs_solutions.at(t).at(aa_pos_i);
                    ...
                    this->map_var_name_to_val[codon_x_bases] = 1;
                    this->map_var_name_to_val[codon_y_bases] = 1;
                }
            }
        }
    }

    std::vector<std::vector<std::string>> generate_shifted_sequences(std::string cycle_start_seq, int n)
    {
        int cycle_length = n;
        int reduced_codon_index = 0;

        // Determine the starting indices from the input DNA sequence
        std::vector<int> start_indices;
        for (size_t i = 0; i < this->instance.decidable_protein_length; ++i) {
            const char aa = this->decidable_protein.at(i);
            const int reduced_codon_len = this->tables.reduced_codon_lengths_table.at(aa);

            std::string codon = cycle_start_seq.substr(reduced_codon_index, reduced_codon_len);
            reduced_codon_index += reduced_codon_len;

            const std::vector<std::string>& codons = this->tables.reduced_codon_table.at(aa);
            
            auto it = std::find(codons.begin(), codons.end(), codon);
            if (it != codons.end()) {
                start_indices.push_back(std::distance(codons.begin(), it));
            } else {
                print_error_newline("Dev Error: input DNA sequence does not match codon table for amino acid " + std::to_string(aa));
                exit(1);
            }
        }

        // Generate shifted codon sequences
        std::vector<std::vector<std::string>> shifted_seqs;
        for (int t = 0; t < cycle_length; ++t)
        {
            ...
            for (size_t i = 0; i < this->instance.decidable_protein_length; ++i)
            {
                ...
                int index = (start_indices[i] + t) % codons.size();
                new_seq.push_back(codons[index]);
            }

            shifted_seqs.push_back(new_seq);
        }
    ...
    // VectorSequenceAminoacidCodonBoolVariables add_base_variables_from_rscu(int n)
    // {

    //     // Variables for each base
    //     VectorSequenceAminoacidCodonBoolVariables sequence_vars_list;

    //     for (int sequence_n = 0; sequence_n < n; ++sequence_n)
    //     {
    ...
    VectorSequenceAminoacidCodonBoolVariables add_base_variables_from_prot(int n)
    {
        // Variables for each base
        VectorSequenceAminoacidCodonBoolVariables sequence_vars_list;
            ...
            for (int amino_acid_position = 0; amino_acid_position < this->decidable_protein_length; ++amino_acid_position)
            {
                ...
                for (size_t codon_number = 0; codon_number < this->tables.reduced_codon_table.at(amino_acid).size(); ++codon_number)
                {
                    ...
                    for (size_t base_idx = 0; base_idx < this->tables.reduced_codon_lengths_table.at(amino_acid); ++base_idx)
                    {
                        ...
                        // print_warning_newline(var_name);
                    }
                    codon_vars_list.push_back(base_vars_list);
                }
                this_sequence_vars_list_of_list.push_back(codon_vars_list);
    ...
    VectorColumnYVars create_y_chained_vars(int n)
    {
        VectorColumnYVars all_pairs_y_terms;
    ...
    std::vector<std::string> get_solver_output_seqs(int n)
    {
        ...
        for (const auto &a : this->sequence_vars_list)
            for (const auto &b : a)
                for (const auto &c : b)
                    for (const auto &var : c)
        ...
        {
            if (operations_research::sat::SolutionIntegerValue(this->response, var))
            {
        ...
        for (int seq_n = 0; seq_n < n; ++seq_n)
        {
            std::string this_seq = "";
            for (int i = 0; i < this->dna_size; ++i)
            {
                if (this->dna_with_holes[i] == '_')
                {
    ...
    std::vector<std::string> get_solver_output_full_seqs()
    {
        std::queue<char> int_vars;
        ...
        std::vector<std::string> all_out_seqs;
        for (int seq_n = 0; seq_n < this->instance.n; ++seq_n)
        {
            std::string this_seq = "";
            for (int i = 0; i < this->dna_size; ++i)
            {
                ...
                {
                    this_seq += this->dna_with_holes[i];
                }
            }
            all_out_seqs.push_back(this_seq);
        }
...
int main(int argc, char *argv[])
{
    ...
    std::string initial_sequence = get_initial_sequence(instance.decidable_protein, tables.reduced_codon_table);

    std::vector<int> sizes;
    for (const char aa : instance.decidable_protein) {
        sizes.push_back(tables.reduced_codon_table[aa].size());
    }
    int cycle_len = lcm_vector(sizes);

    std::vector<int> chunk_sizes;
    for (int i = 0; i < instance.n / cycle_len; ++i)
    {
        chunk_sizes.push_back(cycle_len);  // Full cycles
    }
    if (instance.n % cycle_len)
    {
        chunk_sizes.push_back(instance.n % cycle_len);
    }

    int n_so_far = 0;
    for (int chunk_size_i = 0; chunk_size_i < chunk_sizes.size(); ++chunk_size_i)
    {
        print_warning_newline(initial_sequence);
        int chunk_size = chunk_sizes.at(chunk_size_i);
        n_so_far += chunk_size;
        ...
        bool find_next_cycle_start = (chunk_size_i != chunk_sizes.size() - 1);

        sirius_solver.init_new_model();
        sirius_solver.build_model(initial_sequence, n_so_far, chunk_size, find_next_cycle_start);
        ...
        if (find_next_cycle_start)
        {
            initial_sequence = sirius_solver.get_solver_output_seqs(n_so_far + 1).back();
        }
    }

    std::vector<std::string> all_out_seqs = sirius_solver.get_solver_output_full_seqs();
...
void print_info_newline_time(const std::string &message)
{
    if (!quiet)
    {
        std::cout << BLUE << "> " << RESET "[" << elapsed_since_start() << "] " << message << "\n";
    }
}
...
std::string get_initial_sequence(std::string prot_seq, std::unordered_map<char, std::vector<std::string>> reduced_codon_table)
{
    ...
    for (const char p : prot_seq)
    {
        if (!reduced_codon_table.at(p).at(0).empty())
        {
            output += reduced_codon_table.at(p).at(0);
        }
    }
...
void write_sequences_to_file_and_console(const std::vector<std::string> &sequences, const std::string &base_filename)
{
    ...
    if (!out_file)
    {
        print_error_newline("Failed to open output file.");
        return;
    }
    ...
    {
        too_large = true;
        print_info_newline("Resulting sequences too long to display here.");
    }
    ...
    out_file.close();
    print_info_newline("Find your sequences in " + filename);
}
                                  ...
                                  const SIRIUSTables &tables)
{
    for (size_t i = 0; i < sequences.size(); ++i)
    {
        ...
        {
            print_error_newline("Dev error: SEQ " + std::to_string(i) + " " + sequences[i] + " NOT THE SAME");
        }
    }
}
...
void print_length_counts(const std::unordered_map<int, int> &length_counts, std::ostream *file_out)
{
    ...
    print_info_newline("Fragment length counts:");
    for (const auto &[length, count] : sorted_counts)
    {
        print_info_newline("Length " + std::to_string(length) + ": " + std::to_string(count) + " occurrences");
        if (file_out)
        {
...
void check_response(const operations_research::sat::CpSolverResponse &response)
{
    ...
    {
        print_info_newline_time("Solution is optimal.");
